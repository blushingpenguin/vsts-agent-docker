...
docker run -v /var/run/docker.sock:/var/run/docker.sock --env VSTS_TOKEN=$VSTS_TOKEN --env VSTS_ACCOUNT=$VSTS_ACCOUNT --env VSTS_POOL="Azure Container Instance" --env VSTS_AGENT="ltest" -m 3.5G -d --rm vendeq.azurecr.io/vsts-agent
